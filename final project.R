...
plot <- plot_ly() %>%
  add_trace(
    ...
    marker = list(color = "#c2c1be", size = 12, opacity = 0.7),
    name = "其他城市"
  ) %>%
  add_trace(
    data = data %>% filter(label == "全國"),
    x = ~house, y = ~birth, type = 'scatter', mode = 'markers',
    marker = list(color = "#c47f79", size = 12, opacity = 0.7),
    name = "全國"
  ) %>%
  add_trace(
    data = data %>% filter(label == "台北市"),
    x = ~house, y = ~birth, type = 'scatter', mode = 'markers',
    marker = list(color = "#99a889", size = 12, opacity = 0.7),
    name = "台北市"
  ) %>%
  layout(
    xaxis = list(title = "房價所得比", showline = TRUE, zeroline = TRUE),
    yaxis = list(title = "生育率", showline = TRUE, zeroline = TRUE, titlefont = list(angle = -90)),
    showlegend = TRUE,
    title = list(
      text = "<b>生育率與房價所得比的關聯</b><br><span style='font-size:15px;'>台灣各縣市近二十年</span>",
      x = 0.01,
      y = 0.97,
      xanchor = 'left',
      yanchor = 'top',
...
# 全國迴歸線
data_national <- data %>% filter(city == "全國迴歸線")
...
plot <- plot %>%
  add_lines(
    ...
    name = "全國", line = list(color = '#c47f79', width = 3)
  )

# 台北迴歸線
data_taipei <- data %>% filter(city == "台北市迴歸線")
...
plot <- plot %>%
  add_lines(
    ...
    name = "台北市", line = list(color = '#99a889', width = 3)
  )
